  ...
  return function<T extends () => void>(
    ...
    return {
      value(this: { css?: string; shadyCss?: string } & object) {
        ...
        // tslint:disable-next-line:no-invalid-this
        return [h('style', null, css)].concat(
          descriptor.value.call(this, this)
        );
      }
    };
  };
...
export function prop(config: PropConfig = {}): PropertyDecorator {
  return function(
    targetProto: { [key: string]: any } & Component<any>,
    propertyKey: string | symbol
  ) {
...
export class EventEmitter<T> {
  ...
  constructor(private eventConfig: object = {}) {}

  emit(data: T): boolean {
    return emit(this.origin, this.eventName, {
      ...this.eventConfig,
      ...{ detail: data }
    });
  }
    ...
    Object.defineProperties(proto, {
      ...
      renderCallback: {
        value(this: { shadyCss: string }, ...args: any[]) {
          // tslint:disable-next-line:no-invalid-this
          return [h('style', null, this.shadyCss)].concat(
            originalRenderCallback.apply(this, args)
          );
        }
      }
    });
